...
@dataclass
class MDPOConfig(TrainingArguments):
    r"""
    ...
    overtime_conf: bool = field(
        default=False,
        metadata={
            "help": "Whether to use the new inference strategy with overtime confidence"
        },
    )
    sample_train_steps: int = field(
        default=16,
        metadata={
            "help": "Number of diffusion steps to sample in training, usually in the range of [2, diffusion_steps]."
        },
    )
    block_length: Optional[int] = field(
        default=max_completion_length,
        metadata={
            "help": "Block size for semi-autoregressive inference, default to max_completion_length which disables semi-ar"
        },
    )
